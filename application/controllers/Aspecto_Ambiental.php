...
class Aspecto_Ambiental extends CI_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('Model_AspectoAmbiental','ambiental');
    }
    ...
    {
        $obj = new ent_aspecto_ambiental();
        $obj->asp_id = isset($post['id'])?$post['id']:null;
        $obj->asp_nombre = $post['txtNombre'];
        $obj->cat_id_categoria = +$post['txtIdCategoria'];
        $obj->asp_estado = strtolower($post['txtEstado']) == 'true' ? 1 : 0;
        return $obj;
    }
    public function insertar()
    {
        ...
        $res['resultado'] = $this->ambiental->insertar($obj);
    ...
    public function eliminar($id)
    {
        ...
        $res['resultado'] = $this->ambiental->eliminar($id);
    ...
    public function actualizar()
    {
        ...
        $post = $this->input->post();
        $obj = $this->set_values($post);
        $res['resultado'] = $this->ambiental->actualizar($obj);
        echo json_encode($res);
    }
    public function data($id)
    {
        ...
        $res['objRes'] = $this->ambiental->get_obj($obj);
        if($res['objRes']!=null){
            $res['resultado'] = true;
    ...
    public function datatables_list()
    {
        ...
        $_POST['l'] = $likes;
        $joins = array(
                array('tbl_categorias', 'tbl_categorias.cat_id = tbl_aspecto_ambiental.cat_id_categoria')
        ...
        $datas = $this->Model_Datatables->get_datatables('tbl_aspecto_ambiental','ent_aspecto_ambiental',$likes, $filter,$selects, $order,$joins,$_POST['length'],$_POST['start']);
