...
class Sacp extends CI_Controller {
	 ...
	 public function revisar_sacp($id_sacp = null)
    {
       ...
       $id_unidad    =	$this->session->userdata('id_unidad');	
		...
		{		
			...
			date_default_timezone_set('America/Lima'); //establece la fecha para -5			
			$fecha = new DateTime();
			$ano = $fecha->format('Y');	 //capturo el ano actual
			$datos['lstAnos'] = array($ano , $ano+1);
			
			//maximo tamaño en bytes permitido en el servidor
			$max_post_length = (int)(str_replace('M', '', ini_get('post_max_size')) * 1024 * 1024);		
			$datos['maximo_megas'] = round($max_post_length / (1024*1024), 2);
			
			
			//print_rr($datos['lstAnos'] );
    ...
    //PASO 6 Agregar acción CORRECTIVA / PREVENTIVA
    public function agregar_accion_correctiva_preventiva()  {
    ...
    //PASO 6 Agregar acción CORRECTIVA / PREVENTIVA -- PARA LA SUBIDA DE FICHEROS
    public function subir_fiche_accion_correctiva () {
        ...
        if(isset($_FILES['archivo'])) {
            ...
            $config['upload_path'] = 'docs/evidencia_sacp/';
        ...
        {                   
            ...
            $id_sacp = $post['identificador']; //
            $nom_fichero = $nombre; //asignado arriba, al guardar el fichero
           //Fecha Reporte Para que el usuario no cambie del input, readonly
            date_default_timezone_set('America/Lima'); //establece la fecha para -5         
            $fecha = new DateTime();
            $fecha_subida = $fecha->format('Y-m-d'); //--->Esto irá a la BD
            
                    
            $this->load->model('Model_Sacp');
            $id_evidencia = $this->Model_Sacp->insertar_evidencia($nom_fichero,$fecha_subida);
            
            $identificador_accion = uniqid();
