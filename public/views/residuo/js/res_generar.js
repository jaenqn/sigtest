...
var tpl_fuente = `   <tr>
                        ...
                        <td style="display:flex" class="fuenteGenTR"><input type="text" name="txtFuenGenTR[]" class="txtFuenGenTR" value="" placeholder=""></td>
                      </tr>`;
var tpl_fuente_dos =    `
                        <tr>
...
function fnSubmitDeclara(e){
    e.preventDefault();
    $.post(BASE_URL + 'residuo/registrar_declaracion', rg.frmDeclara.serializeArray(), function(data, textStatus, xhr) {
        // console.log(data);
...
function fnChangeinst(e){
    ...
    $.post(BASE_URL + 'residuo/get_res_declaracion?tipo=3', {
        idInstalacion   : id_ins
    }, function(data, textStatus, xhr) {
        console.log(data);
        var t = Handlebars.compile(TEMPLATES.option);
        var html = t({id : '', text : ''});
        data.forEach(function(e,i){
            html += t({id : e.get_year, text : e.get_year})
        });
        rg.selResiduoYear.html(html);
        if(data.length > 0)
            rg.selResiduoYear.val(data[0].get_year).trigger('change');
    },'json').fail(fnFailAjax);
    loadDataResiduos();
