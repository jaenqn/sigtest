...
function fnLoad(){
    ...
    dtListar = tblListar.DataTable({
        ...
        order : [0,'asc'],
        ajax : {
            "url": BASE_URL + "residuo/datatables_declaracions",
            "type": "POST",
            data : function(d){
        ...
        columns : [
            ...
            { data : "res_nombre"},
            { data : "rd_year" },
            { data : "rd_id",render:function(data, type, row){
                var t = '<a href="{{urla}}"  data-id-obj="{{id}}" class="  btn btn-default btn-option-files" data-accion="editar" style=""  title="Editar" ><i class="fa fa-pencil" style="font-size:1em"></i></a><a  data-id-obj="{{id}}" class="btnAcciones btn btn-default btn-option-files" data-accion="eliminar" style=""  title="Eliminar" ><i class="fa fa-trash" style="font-size:1em"></i></a><a href="{{urlb}}" data-id-obj="{{id}}" class="btn btn-default btn-option-files" data-accion="word" style=";background: rgb(35, 114, 186);border: 0px solid rgb(35,114,186);padding: 0px 4px;"  title="Descargar en Word" ><img src="'+BASE_URL+'public/app/img/iconos/word.png"  style="width: 20px;"></a>';
                t = Handlebars.compile(t);
                return t({id : data,
                    urla : BASE_URL + 'residuo/declaracion/' + data,
                    urlb : BASE_URL + 'residuo/declaracion/' + data + '?download=word'
                });
            } }
    ...
    function fnEliminar(id,element){
        ...
        swal({
            ...
            text: "¿Desea eliminar esta Declaración?",
            type: "warning",
        ...
        function(isConfirm){
          if (isConfirm) {
            $.post(BASE_URL + 'residuo/eliminar_declaracion/' + id, {}, function(data, textStatus, xhr) {
                console.log(data);

                if(data.resultado == true){
                    swal("Eliminado!", "La Declaración fue eliminada", "success");
                    $(element).parents('tr').remove();
                    // dtListar.row($(element).toParent('tr')).remove().draw();
                }else{
                    swal("Cancelado!", "La Declaración no se puede eliminar", "error");
                }
            },'json').fail(fnFailAjax);
          }
        });
    ...
    function fnBuscar(e){
        ...
        if(frmF.selResiduoF.value > 0)
            filtrosDatatable.push({
                column : 'res_id_residuo',
                filter : frmF.selResiduoF.value});

        if(frmF.selYearF.value >= 0)
            filtrosDatatable.push({
                column : 'rd_year',
                filter : frmF.selYearF.value});


        if(frmF.txtInstalacionF.value.trim() != '')
            filtrosDatatable.push({
                column : 'ins_nombre',
                filter : frmF.txtInstalacionF.value});
