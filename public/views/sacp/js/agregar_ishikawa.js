$(document).ready(function() {
    ...
    /***********--DELETE--***************/
    //PARA GENERAL DE CAUSAS (5)
    $("#tbl_causas_todas").on('click', 'a.delete', function(e) {
        var id = $(this).parent().parent().attr('id');
        //alert(id);exit;
        var b = $(this).parent().parent();
        var dataString = 'id=' + id;
        ...
        swal({
            ...
            function() {
                ...
                $.ajax({
                    ...
                    success: function(e) {
                        llenar_tabla_causas();
                        $('.modal_agregar_causa').modal('hide');
                    }
                });
                return false;
...
function llenar_tabla_causas() {
    ...
    $.ajax({
        type: "POST",
        url: BASE_URL + "sacp/listado_causas_xSacp",
        data: dataString,
        dataType: 'json',
        cache: false,
        complete: function() {
        ...
        success: function(datos) {
            ...
            for (i = 0; i < datos.length; i++) {
                ...
                if (datos[i].sacishi_tipo == 0) {
                    tabledata += "<a class='ver_modal' href='#tab_content2_cinco'  data-toggle='modal' data-target='.modal_agregar_causa'>" +
                        "<img src='" + BASE_URL + "public/views/sacp/img/text_preview.png' width='20px'></img> " +
                        "</a> &nbsp;&nbsp;";

                } else {
                    tabledata += "<a class='ver_modal' href='#'  data-toggle='modal' data-target='.modal_agregar_causa'>" +
                        "<img src='" + BASE_URL + "public/views/sacp/img/text_preview.png' width='20px'></img> " +
                        "</a> &nbsp;&nbsp;";
                }
            ...
            $('#tbl_causas_todas tbody').html(tabledata);
